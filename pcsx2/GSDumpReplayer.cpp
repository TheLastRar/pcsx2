...
void GSDumpReplayer::RenderUI()
{
	...
	std::pair<ImFont*, float> font = ImGuiManager::GetFixedFont();
	std::string text;
	ImVec2 text_size;
	text.reserve(128);
...
#define DRAW_LINE(font, text, color) \
	...
	{ \
		...
		dl->AddText(font.first, font.second, ImVec2(margin + shadow_offset, position_y + shadow_offset), IM_COL32(0, 0, 0, 100), (text)); \
		dl->AddText(font.first, font.second, ImVec2(margin, position_y), color, (text)); \
