...
{
#ifdef _WIN32
	SOCKET CreateSocket(std::optional<PacketReader::IP::IP_Address> adapterIP)
	{
		SOCKET client = INVALID_SOCKET;
#elif defined(__POSIX__)
	int CreateSocket(std::optional<PacketReader::IP::IP_Address> adapterIP)
	{
		...
		if (adapterIP.has_value())
		{
			...
			endpoint.sin_addr = std::bit_cast<in_addr>(adapterIP.value());
