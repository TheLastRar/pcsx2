...
namespace Sessions
{
	...
	IP_Payload* UDP_Session::Recv()
	{
		...
		if (hasData)
		{
			...
			if (ret != SOCKET_ERROR)
			{
				recived = new PayloadData(available);
...
#endif
				ret = recvfrom(client, (char*)recived->data.get(), available, 0, &endpoint, &fromlen);
			}
	...
	bool UDP_Session::Send(PacketReader::IP::IP_Payload* payload)
	{
		...
		if (destPort != 0)
		{
			...
			{
				Console.Error("UDP packet invalid for current session (Duplicate key?)");
				return false;
			}
		}
		else
		{
			...
			//			const BOOL keepPortUnreacable = false;
			//			ret = WSAIoctl(client, SIO_UDP_CONNRESET, (LPVOID)&keepPortUnreacable, sizeof(keepPortUnreacable), nullptr, 0, nullptr, nullptr, nullptr);
			//#endif
