...
#define HEX_TO_IMVEC4(hex, alpha) \
	...
	extern ImGuiManager::Font g_standard_font;
	extern ImGuiManager::Font g_medium_font;
	extern ImGuiManager::Font g_large_font;
	...
	bool FloatingButton(const char* text, float x, float y, float width = -1.0f, float height = LAYOUT_MENU_BUTTON_HEIGHT_NO_SUMMARY,
		float anchor_x = 0.0f, float anchor_y = 0.0f, bool enabled = true, ImGuiManager::Font font = g_large_font, ImVec2* out_position = nullptr,
	...
	bool EnumChoiceButtonImpl(const char* title, const char* summary, s32* value_pointer,
		const char* (*to_display_name_function)(s32 value, void* opaque), void* opaque, u32 count, bool enabled, float height, ImGuiManager::Font font,
		ImGuiManager::Font summary_font);
	...
	void EndNavBar();
	void NavTitle(const char* title, float height = LAYOUT_MENU_BUTTON_HEIGHT_NO_SUMMARY, ImGuiManager::Font font = g_large_font);
