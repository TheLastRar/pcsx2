	...
	*menu_size = ImVec2(io.DisplaySize.x, io.DisplaySize.y - heading_size.y - LayoutScale(LAYOUT_FOOTER_HEIGHT));
	...
	if (BeginFullscreenWindow(ImVec2(0.0f, 0.0f), heading_size, "landing_heading", UIPrimaryColor))
	{
		...
		// draw branding
		{
			...
			dl->AddImage(reinterpret_cast<ImTextureUserID>(GetCachedTexture("icons/AppIconLarge.png")->GetNativeHandle()),
		...
		if (Achievements::IsActive())
		{
			...
			if (username)
			{
				...
				if (!badge_path.empty()) [[likely]]
				{
					...
					dl->AddImage(reinterpret_cast<ImTextureUserID>(GetCachedTextureAsync(badge_path)->GetNativeHandle()),
