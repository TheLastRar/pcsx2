		...
		if (IsMipMapActive() && GSConfig.HWMipmap && !tex_psm.depth && !src->m_from_hash_cache)
		{
			...
			for (int layer = layer_levels.x; layer <= layer_levels.y; layer++)
			{
				...
				else
				{
					...
					if (tgt == nullptr)
						tgt = g_texture_cache->CreateTarget(MIP_TEX0, GSVector2i(1 << MIP_TEX0.TW, 1 << MIP_TEX0.TH), GSVector2i(1 << MIP_TEX0.TW, 1 << MIP_TEX0.TH), 1, GSTextureCache::RenderTarget, true, 0, false, true, true);
