...
bool GSDevice11::CreateLibrashaderFilterChain(const std::string& preset_path)
{
	...
	if (err)
	{
		...
		{
			Console.ErrorFmt("librashader: D3D11 chain create failed for '{}': {} (errno={})", preset_path, error_message, static_cast<int>(error_code));
			libra_error_free_string(&error_message);
		}
		else
		{
			Console.ErrorFmt("librashader: D3D11 chain create failed for '{}': unknown error (errno={})", preset_path, static_cast<int>(error_code));
		}
		libra_error_free(&err);
