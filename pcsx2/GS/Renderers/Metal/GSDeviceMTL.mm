...
{ @autoreleasepool {
	...
	if (err)
	{
		...
		{
			Console.ErrorFmt("librashader: Metal chain create failed for '{}': {} (errno={})", preset_path, error_message, static_cast<int>(error_code));
			libra_error_free_string(&error_message);
		}
		else
		{
			Console.ErrorFmt("librashader: Metal chain create failed for '{}': unknown error (errno={})", preset_path, static_cast<int>(error_code));
		}
		libra_error_free(&err);
	...
	if (!chain)
	{
		Console.ErrorFmt("librashader: Metal chain create failed for '{}': no error returned and no chain created", preset_path);
		return false;
	}
...
{ @autoreleasepool {
	...
	{
		libra_mtl_filter_chain_t chain = static_cast<libra_mtl_filter_chain_t>(m_librashader_chain);
...
{ @autoreleasepool {
	...
	// can't have a stray blit encoder open when librashader spins up its own render pass on this cmdbuf.
	if (m_late_texture_upload_encoder)
	{
		[m_late_texture_upload_encoder endEncoding];
		m_late_texture_upload_encoder = nullptr;
...
void GSDeviceMTL::ApplyLibrashaderChainParams(const std::vector<std::pair<std::string, float>>& params)
{ @autoreleasepool {
	if (!m_librashader_chain)
		return;
	libra_mtl_filter_chain_t chain = static_cast<libra_mtl_filter_chain_t>(m_librashader_chain);
	...
	{
		libra_error_t err = libra_mtl_filter_chain_set_param(&chain, name.c_str(), value);
		if (err)
			libra_error_free(&err);
	}
} }
