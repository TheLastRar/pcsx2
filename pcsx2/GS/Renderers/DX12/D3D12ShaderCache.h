...
public:
	...
	__fi ComPtr<ID3DBlob> GetPixelShader(
		const std::vector<std::string>& shader_code, const D3D_SHADER_MACRO* macros = nullptr, const char* entry_point = "main", const std::unordered_map<std::string, std::string>& includes = {})
	{
		return GetShaderBlob(EntryType::PixelShader, shader_code, macros, entry_point, includes);
	}
	...
	ComPtr<ID3DBlob> GetShaderBlob(EntryType type, const std::vector<std::string>& shader_code,
		const D3D_SHADER_MACRO* macros = nullptr, const char* entry_point = "main", const std::unordered_map<std::string, std::string>& includes = {});
...
private:
	...
	ComPtr<ID3DBlob> CompileAndAddShaderBlob(const CacheIndexKey& key, const std::vector<std::string>& shader_code,
