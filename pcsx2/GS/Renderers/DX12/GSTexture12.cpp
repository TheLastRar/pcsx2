	...
	DXGI_FORMAT uav_format)
{
	...
	switch (type)
	{
		...
		case Type::DepthStencil:
		{
			...
			if (!CreateReadDSVDescriptor(resource.get(), dsv_format, &ro_dsv_descriptor))
			{
...
bool GSTexture12::CreateReadDSVDescriptor(ID3D12Resource* resource, DXGI_FORMAT format, D3D12DescriptorHandle* dh)
{
	if (!GSDevice12::GetInstance()->GetDSVHeapManager().Allocate(dh))
	{
		Console.Error("Failed to allocate SRV descriptor");
	...
	const D3D12_DEPTH_STENCIL_VIEW_DESC desc = {format, D3D12_DSV_DIMENSION_TEXTURE2D, D3D12_DSV_FLAG_READ_ONLY_DEPTH};
	GSDevice12::GetInstance()->GetDevice()->CreateDepthStencilView(resource, &desc, dh->cpu_handle);
	return true;
}
