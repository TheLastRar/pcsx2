...
void D3D12StreamBuffer::FlushMemory()
{
		...
		m_current_space != m_size)
	{
		if (m_current_copy_offset < m_current_offset)
		{
			...
			GSDevice12::GetInstance()->GetCopyCommandList().list4->CopyBufferRegion(m_buffer_default.get(), m_current_copy_offset,
		...
		else
		{
			...
			GSDevice12::GetInstance()->GetCopyCommandList().list4->CopyBufferRegion(m_buffer_default.get(), m_current_copy_offset,
				m_buffer_upload.get(), m_current_copy_offset, size);
			GSDevice12::GetInstance()->GetCopyCommandList().list4->CopyBufferRegion(m_buffer_default.get(), 0,
