...
bool GSDeviceVK::CreateDevice(VkSurfaceKHR surface, bool enable_validation_layer)
{
	...
	std::vector<int> queue_family_usage(queue_family_count);
	...
	for (uint32_t i = 0; i < queue_family_count; i++)
	{
		if (queue_family_properties[i].queueCount == queue_family_usage[i])
			continue;
	...
	{
		spin_queue_index = queue_family_usage[m_spin_queue_family_index];
		queue_family_usage[m_spin_queue_family_index]++;
		m_spin_queue_is_graphics_queue = false;
	}
	else
	{
		// Try the graphics queue
	...
	if (surface)
	{
		...
		{
			present_queue_index = queue_family_usage[m_present_queue_family_index];
			queue_family_usage[m_present_queue_family_index]++;
		}
		else
		{
			// Use graphics queue for present if no spare queues found
			VkBool32 present_supported;
	...
	// Swap spin and present to simplify later logic
	...
	graphics_queue_info.queueCount = queue_family_usage[m_graphics_queue_family_index];
	...
	if (surface != VK_NULL_HANDLE && m_graphics_queue_family_index != m_present_queue_family_index)
	{
		...
		present_queue_info.queueCount = queue_family_usage[m_present_queue_family_index];
