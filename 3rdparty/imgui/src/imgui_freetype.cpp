...
// Updated with https://github.com/sammycage/lunasvg/issues/150#issuecomment-2381364213
// Renders smaller than plutoSVG
struct ImGuiLunasvgPortEntry
{
	FT_UShort start_glyph_id;
	FT_UShort end_glyph_id;
	std::unique_ptr<lunasvg::Document, lunasvg::Document::DocumentDeleter> document;
};
...
static lunasvg::Document* ImGuiLunasvgPortLoad(LunasvgPortState* state, FT_SVG_Document ft_document, FT_UInt index)
{
	...
	if (document == nullptr)
		return nullptr;
	state->entries.emplace_back(ImGuiLunasvgPortEntry{ft_document->start_glyph_id, ft_document->end_glyph_id, std::move(document)});
...
{
	std::string value = element.getAttribute("width");
	if (value.empty())
		return false;
	assert(false);
...
static void ImGuiLunasvgGetPlutoSize(lunasvg::Document* document, float& width, float& height)
{
	...
	std::string vb = rootElement.getAttribute("width");
