...
rem Add ARM64EC support
%PATCH% -p1 < "%SCRIPTDIR%\libpng-arm64ec.patch" || goto error
...
cd "lz4" || goto error
cmake %ARM64TOOLCHAIN% -DCMAKE_BUILD_TYPE=Release -DCMAKE_PREFIX_PATH="%INSTALLDIR%" -DCMAKE_INSTALL_PREFIX="%INSTALLDIR%" -DBUILD_SHARED_LIBS=ON -DLZ4_BUILD_CLI=OFF -DLZ4_BUILD_LEGACY_LZ4C=OFF -DCMAKE_C_FLAGS="/arm64EC /wd4711 /wd5045" -B build-dir -G Ninja build/cmake || goto error
...
cd "%SDL%" || goto error
cmake -B build %ARM64TOOLCHAIN% -DCMAKE_BUILD_TYPE=Release %FORCEPDB% -DCMAKE_INSTALL_PREFIX="%INSTALLDIR%" -DBUILD_SHARED_LIBS=ON -DSDL_SHARED=ON -DSDL_STATIC=OFF -DSDL_LIBC=ON -G Ninja || goto error
cmake --build build --parallel || goto error
ninja -C build install || goto error
...
rem ARM64EC support for Qt is missing
rem https://bugreports.qt.io/browse/QTBUG-125260
rem https://bugreports.qt.io/browse/QTBUG-113583
rem Will need to use x64 builds
