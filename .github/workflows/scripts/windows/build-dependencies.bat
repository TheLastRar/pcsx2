...
call :downloadfile "lpng%LIBPNG%-apng.gz" https://download.sourceforge.net/libpng-apng/libpng-%LIBPNGLONG%-apng.patch.gz b68e523146dc326d6fd3cd49d55f9ca2d73f4a4d6e3d0f095bcc8988ffe8db2b || goto error
...
rem rem apng not in released libpng yet
rem %SEVENZIP% x "lpng%LIBPNG%-apng.gz" -aoa || goto error
rem cd "lpng%LIBPNG%" || goto error
rem %PATCH% -p1 < "../libpng-%LIBPNGLONG%-apng.patch" || goto error
...
rem TODO, Figure out how to name qtapng as qtapngd for debug
rem TODO, install dosn't install anything
echo Building Qt APNG...
rmdir /S /Q "qtapng-%QTAPNG%"
%SEVENZIP% x "qtapng-%QTAPNG%.zip" || goto error
cd "qtapng-%QTAPNG%" || goto error
cmake -B build -DCMAKE_PREFIX_PATH="%INSTALLDIR%" -DCMAKE_INSTALL_PREFIX="%INSTALLDIR%" %QTAPNGBUILDSPEC% || goto error
