...
void QtHost::UpdateGlyphRangesAndClearCache(QWidget* dialog_parent, const std::string_view language)
{
	...
	if (g_emu_thread)
	{
		Host::RunOnCPUThread([font_path = std::move(font_path), fallback_fonts_paths = std::move(fallback_fonts_paths)]() mutable {
			if (MTGS::IsOpen())
			{
				MTGS::RunOnGSThread([font_path = std::move(font_path), fallback_fonts_paths = std::move(fallback_fonts_paths)]() mutable {
					ImGuiManager::SetFontPath(std::move(font_path), std::move(fallback_fonts_paths));
				});
			}
			else
			{
