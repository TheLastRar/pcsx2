...
{
	const std::optional<int> portLengthOpt = m_dialog->getIntValue("DEV9/Eth/Ports", "Count", std::nullopt);
	if (!portLengthOpt.has_value())
	{
		...
		{
			case QMessageBox::StandardButton::No:
				m_dialog->setIntSettingValue("DEV9/Eth/Ports", "Count", 0);
				break;
	...
	else
	{
		...
		m_dialog->setIntSettingValue("DEV9/Eth/Ports", nullptr, std::nullopt);
	}
...
void DEV9SettingsWidget::onEthPortEdit(QStandardItem* item)
{
	...
	switch (item->column())
	{
		case 0: //Name
			...
			break;
		case 1: //Protocol
			m_dialog->setStringSettingValue(section.c_str(), "Protocol", item->text().toUtf8().constData());
			break;
		case 2: //Port
			m_dialog->setIntSettingValue(section.c_str(), "Port", item->data(Qt::EditRole).toInt());
			break;
		case 3: //Enabled
			m_dialog->setBoolSettingValue(section.c_str(), "Enabled", item->checkState() == Qt::CheckState::Checked);
			break;
...
{
	return m_dialog->getIntValue("DEV9/Eth/Ports", "Count", 0).value();
}
...
std::optional<std::vector<PortEntryUi>> DEV9SettingsWidget::ListPortsConfig()
{
	...
	if (m_dialog->isPerGameSettings())
	{
		hostLengthOpt = m_dialog->getIntValue("DEV9/Eth/Ports", "Count", std::nullopt);
		if (!hostLengthOpt.has_value())
			return std::nullopt;
	}
	else
		hostLengthOpt = m_dialog->getIntValue("DEV9/Eth/Ports", "Count", 0);
	...
	for (int i = 0; i < hostLength; i++)
	{
		...
		PortEntryUi entry;
		entry.Protocol = m_dialog->getStringValue(section.c_str(), "Protocol", "UDP").value();
		entry.Desc = m_dialog->getStringValue(section.c_str(), "Desc", "").value();
		entry.Port = m_dialog->getIntValue(section.c_str(), "Port", 0).value();
		entry.Enabled = m_dialog->getBoolValue(section.c_str(), "Enabled", false).value();
...
void DEV9SettingsWidget::AddNewPortConfig(const PortEntryUi& port)
{
	...
	// clang-format off
	m_dialog->setIntSettingValue   (section.c_str(), "Port",     port.Port);
	m_dialog->setStringSettingValue(section.c_str(), "Desc",     port.Desc.c_str());
	m_dialog->setStringSettingValue(section.c_str(), "Protocol", port.Protocol.c_str());
	m_dialog->setBoolSettingValue  (section.c_str(), "Enabled",  port.Enabled);
	// clang-format on
	m_dialog->setIntSettingValue("DEV9/Eth/Ports", "Count", portLength + 1);
	RefreshPortList();
}
...
void DEV9SettingsWidget::DeletePortConfig(int index)
{
	...
	for (int i = index; i < portLength - 1; i++)
	{
		...
		// clang-format off
		m_dialog->setIntSettingValue   (section.c_str(), "Port",     m_dialog->getIntValue   (sectionAhead.c_str(), "Url",     0).value());
		m_dialog->setStringSettingValue(section.c_str(), "Desc",     m_dialog->getStringValue(sectionAhead.c_str(), "Desc",    "").value().c_str());
		m_dialog->setStringSettingValue(section.c_str(), "Protocol", m_dialog->getStringValue(sectionAhead.c_str(), "Address", "UDP").value().c_str());
		m_dialog->setBoolSettingValue  (section.c_str(), "Enabled",	 m_dialog->getBoolValue  (sectionAhead.c_str(), "Enabled", false).value());
		// clang-format on
	}
